...
def process_image(image: np.ndarray, target: str, background_class: str, segmentation_threshold: float,
    ...
    # fg_text_features = CLIPer.cliper.classifier(blip_img, fg_classes)
    # bg_text_features = CLIPer.cliper.classifier(blip_img, bg_classes)
    bg_text_features = torch.cat([bg_text_features,
                                  embedding[(fg_text_features @ embedding.T).sort().indices[:, ::1500]].flatten(0, 1)],
                                 dim=0)
    pred_mask, final_score = CLIPer(
        ori_img.to(device), image.to(device),
        fg_text_features, bg_text_features,
        classes_name=fg_classes, blip_img=blip_img
    )
