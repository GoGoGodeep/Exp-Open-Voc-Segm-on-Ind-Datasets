...
class CLIPer_BLIP(nn.Module):
    def __init__(
            self, model_name="ViT-L/14", attn_type="fused-attn",
            fuse_feature=True, logit_scale=100, device="cuda", size=336
    ):
        super(CLIPer_BLIP, self).__init__()
    ...
    def classifier(self, img, classnames):
        ...
        with torch.no_grad():
            ...
            for classname in classnames:
                ...
                # 使用BLIP进行文本-图像输入处理
                inputs = blip_processor(img, text, return_tensors="pt").to(self.device)
                ...
                texts = [text, generated_text]  # 保持两个独立描述
