...
class iSeg(pl.LightningModule):
    ...
    def get_att_map(self, cross_attention_maps, self_attention_maps):
        """
        将 cross-attention 投射到图像空间。
        cross_att 由 process_cross_att() 得到，self_att 用于传播语义上下文。
        """
...
class interaction_iSeg(iSeg):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # 计算语义 token 索引并放大其 embedding（语义增强）
        ...
        #     self.test_t_embedding[:, idx] *= self.config.enhanced
